...
<template>
  <div
    class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col h-full max-h-[calc(100vh-100px)]"
  >
    <h2 class="mb-4 font-bold text-gray-800 shrink-0">ประวัติรายการฝากถอน</h2>

    <div
      class="hidden md:block overflow-auto border border-gray-200 rounded-lg"
    >
      <table class="w-full text-sm text-left border-collapse relative">
        ...
        <tbody class="bg-white">
          <tr
            v-for="item in bankStore.transactions"
            :key="item.id"
          ...
          >
            ...
            <td
              class="p-3 text-gray-500 max-w-[200px] truncate"
              :title="item.email"
            >
              {{ item.email }}
            </td>
            <td class="p-3 flex gap-2">
              <button
                ...
                @click="openEdit(item)"
                class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 transition text-xs shadow-sm"
              >
                <Pencil :size="16" />
              </button>
              <button
                ...
                class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition text-xs shadow-sm"
              >
                <Trash2 :size="16" />
              </button>
            </td>
    ...
    <div class="md:hidden space-y-4 overflow-y-auto max-h-[70vh] pr-1 pb-4">
      <div
        v-for="item in bankStore.transactions"
        :key="item.id"
        class="border border-gray-200 rounded-lg p-4 bg-gray-50 space-y-3 relative shadow-sm"
      >
        ...
        <div class="pt-2 border-t border-gray-200 flex justify-end gap-2">
          <button
            v-if="item.status === 'ฝาก'"
            @click="openEdit(item)"
            class="text-yellow-600 text-sm font-medium hover:underline"
          >
            <Pencil :size="16" />
          </button>
          <button
      ...
      <div
        v-if="bankStore.transactions.length === 0"
        class="text-center p-8 text-gray-400 border border-dashed rounded-lg"
      >
        ยังไม่มีรายการประวัติ
      </div>
    </div>
